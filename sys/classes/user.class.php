...
class User {
    ...
    public function getUserLoginById($userId){
        $qForGetting = "SELECT `login` FROM `users` WHERE `id` = ? LIMIT 1";
        $login = DB::connect()->prepare($qForGetting);
        $login->execute(Array($userId));
        return $login->fetch();
    }
    public function getAllUsers(){
        $qForGetting = "SELECT * FROM `users`";
        $users = DB::connect()->prepare($qForGetting);
        $users->execute();
        return $users;
    }
